...
local function gDuelMenu()
    ...
    MainDop.Paint = function(self, w, h) end
    ...
    gDuel.vgui.txtentry(gDuel.Translate("EnterBetVgui") .. "" .. gDuel.minBet .. "" .. gDuel.Translate("EnterBetVgui2") .. "" .. gDuel.maxBet .. ')', MainAdditional, 0, MainAdditional:GetTall() - 45, MainAdditional:GetWide(), 35, function(self)
    ...
    -- let's get players
    local displayPlayers = function(id)
        plList:Clear()
        local plys = {}
        ...
        for k, v in pairs(plys) do
            if v == nil or IsValid(v) == false or v:IsPlayer() == false or v == LocalPlayer() then continue end
            if gDuel.builderX_Support == true and v:GetBuild() == true then continue end -- Builder-X Support (prevent dueling in build mode)
            ...
            function pnl:Paint(w, h)
                ...
                gDuel.vgui.txt(gDuel.Translate("Money") .. '' .. gDuel.Money.display(gDuel.Money.get(ply) or 0), 15, 55, 12)
            end
            ...
            function pnl.select:Think()
                if curWager == nil or not isnumber(curWager) then
...
net.Receive('gDuel.SendRequest', function()
    ...
    function bg:PaintOver(w, h)
        if not IsValid(dpl) then
            bg:Remove()

            return
        end

        gDuel.vgui.txt(dpl:Name() .. '' .. gDuel.Translate("AcceptVgui") .. '' .. gDuel.Types[type].name .. '' .. gDuel.Translate("AcceptVgui2") .. '' .. gDuel.Money.display(wager), 15, w / 2, 28, nil, 1)
